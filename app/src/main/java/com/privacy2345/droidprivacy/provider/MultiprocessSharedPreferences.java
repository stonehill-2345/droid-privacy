package com.privacy2345.droidprivacy.provider;
...
import com.privacy2345.droidprivacy.BuildConfig;
...
/**
 * 多进程SharedPreferences实现类
 * 使用ContentProvider实现跨进程的SharedPreferences读写功能
 *
 * 实现原理：
 * 1. ContentProvider天生支持多进程访问
 * 2. 使用内部私有BroadcastReceiver实现跨进程的OnSharedPreferenceChangeListener监听
 *
 * 使用方法：在AndroidManifest.xml中添加provider声明：
 * <pre>
 * &lt;provider
 *     android:name="com.privacy2345.droidprivacy.provider.MultiprocessSharedPreferences"
 *     android:authorities="com.privacy.checker.MultiprocessSharedPreferences"
 *     android:process="com.privacy.checker.MultiprocessSharedPreferences"
 *     android:exported="false" /&gt;
 * &lt;!--
 * authorities属性建议使用包名做前缀，APK安装时authorities同名的provider需要校验签名，否则无法安装
 * --&gt;
 * </pre>
 *
 * 注意事项：
 * 1. ContentProvider进程管理：
 *    - 当ContentProvider所在进程被killProcess(pid)时，会导致整个应用程序异常退出或进程重启
 *    - 重启报错信息：Acquiring provider &lt;processName&gt; for user 0: existing object's process dead
 *
 * 2. 安全模式处理：
 *    - 设备处于"安全模式"时，只有系统自带的ContentProvider可用
 *    - 在此模式下，put操作默认返回false，get操作默认返回null
 *
 * 3. 异常处理：
 *    - 需要处理PackageManager死亡等系统异常情况
 *    - 提供完善的异常处理机制，避免进程死亡导致的崩溃
 *
 * 替代方案对比：
 * 使用FileLock和FileObserver也可以实现多进程SharedPreferences读写，
 * 但维护成本高，需要定期对照系统实现更新新特性。
 *
        ...
        switch (mUriMatcher.match(uri)) {
            ...
            case GET_STRING_SET: {
                if (Build.VERSION.SDK_INT >= 11) { // Android 3.0
                    ...
                    bundle.putSerializable(KEY, (HashSet<String>) ReflectionUtil
                        .sharedPreferencesGetStringSet(getSystemSharedPreferences(name, mode), key, set));
                }
            }
            default:
                if (DEBUG) {
